            ...
            <div className={styles.upperSection}>
                ...
                <div className={styles.infoColumn}>
                    ...
                    <div className={styles.actionsBar}>
                        ...
                        <div className={styles.infoBadge}>INFO</div>
                    </div>

                    <div className={styles.detailsBlock}>
                        <span className={styles.blockLabel}>TRACKLIST / PREVIEW</span>
                        {product.tracklist && product.tracklist.length > 0 ? (
                            <ol className={styles.tracklist}>
                                {product.tracklist.map((track, index) => (
                                    <li key={index}>{track.toUpperCase()}</li>
                                ))}
                            </ol>
                        ) : (
                            <p className={styles.noDetails}>INFORMACIÓN DE CONTENIDO NO DISPONIBLE</p>
                        )}

                        <div className={styles.genresBox}>
                            {product.genres?.map((g, i) => (
                                <span key={i} className={styles.genreTag}>#{g.toUpperCase()}</span>
                            ))}
                        </div>
                    </div>
                </div>
